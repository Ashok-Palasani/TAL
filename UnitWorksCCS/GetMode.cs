...
namespace UnitWorksCCS
{
    public class GetMode
    {
        ...
        public string[] Getmode(int MachineID)
        {
            ...
            var Mode = db.tblmodes.Where(m => m.IsDeleted == 0 && m.MachineID == MachineID).OrderByDescending(m => m.ModeID);
            foreach (var val in Mode)
            {
                modefromtable = val.Mode;
        ...
        public void Modetable(string Mode, int Machineid)
        {
            ...
            message.CorrectedDate = Convert.ToString(correctedDate);

            db.tblmodes.Add(message);
            db.SaveChanges();
        }
        ...
        public string GetShift()
        {
            ...
            var shiftdetails = db.tblshift_mstr.Where(m => m.StartTime <= CtTime && m.EndTime >= CtTime);
            if (shiftdetails.Count() != 0)
            {
                foreach (var shft in shiftdetails)
                {
                    Shift = shft.ShiftName.Trim();
                }
            }
            else
            {
        ...
        //        {
        //            Shift = shft.ShiftName.Trim();
        //            Shiftendtime = shft.ShiftName.ToString().Trim();
        //        }
        //    }
        //    else
        //    {
        ...
        //        var O8531 = db.tabprogram_master.Where(m => m.MachineID == Machineid && m.InsertedOn >= datetimebefore10mnt && m.InsertedOn <= EndTime);
        //        int count = 0;
        ...
        //        var shiftdetails1 = db.tabtblshiftdetails.Where(m => m.ShiftDetailsName == Shift);
        ...
        //        DateTime Currenttime = DateTime.Now;
        //        datetimebefore10mnt = EndTime.AddMinutes(-10);
        //        var O8531 = db.tabprogram_master.Where(m => m.MachineID == Machineid && m.InsertedOn >= datetimebefore10mnt && m.InsertedOn <= Currenttime);
        //        int count = 0;
        //        foreach (var data in O8531)
        //        {
        //            count = count + 1;
        ...
        //    var shiftdetails = db.tabtblshiftdetails.Where(m => m.ShiftStartTime <= CtTime && m.ShiftEndTime >= CtTime);
        //    if (shiftdetails.Count() != 0)
        //    {
        //        foreach (var shft in shiftdetails)
        //        {
        ...
        public void GetCorrectedDate(out String correctedDate)
        {
            ...
            SqlDataAdapter daGDS = new SqlDataAdapter(GetDayStartQuery, MC.sqlConnection);
        ...
        //    string mode = null;
        //    int IdleLock = 0;
        //    var IdleEntry = db.tblmodes.Where(m => m.IsDeleted == 0 && m.MachineID == machnid && m.IsCompleted == 0).OrderByDescending(m => m.ModeID).FirstOrDefault();
        //    if(IdleEntry != null)
        //    {
        //        mode = IdleEntry.Mode;
        //        IdleLock = IdleEntry.StartTime;
        ...
        //{
        //    var MacDet = db.tabtblmachinedetails.Where(m => m.MachineID == MacID).FirstOrDefault();
        ...
        //    var MachineDetails = db.tabtblmachinedetails.Where(m => m.TabIPAddress == TabIPAddress && m.IsDeleted == 0).FirstOrDefault();
        //    try
        //    {
        //        ServerReply = ServerPing.Send(MachineDetails.ServerIPAddress, 500);
        ...
        //    var operatorHeader = db.tabtblOperatorHeaders.Where(m => m.MachineID == MacID).OrderByDescending(m => m.InsertedOn).FirstOrDefault();
        ...
        //public void UpdateOperatorHeader(int MacID)
        //{
        //    int MacDet = db.tblmachinedetails.Where(m => m.MachineID == MacID).Select(m => m.MachineID).FirstOrDefault();
        ...
        //    var MachineDetails = db.tblmachinedetails.Where(m => m.MachineID == MacDet).FirstOrDefault();
        //    try
        //    {
        ...
        //    String MacIP = MachineDetails.IPAddress;
        //    Ping MacPing = new Ping();
        //    PingReply MacReply;
        //    MacReply = MacPing.Send(MacIP, 500);
        ...
        //    }
        //    #region Update tblOperatorHeader 
        //    var operatorHeader = db.op.Where(m => m.MachineID == MacID).OrderByDescending(m => m.InsertedOn).FirstOrDefault();
        //    string ModeType = db.tblmodes.Where(m => m.IsDeleted == 0 && m.MachineID == MacID && m.IsCompleted==0).Select(m => m.ModeType).FirstOrDefault();// getting current mode
        ...
        //        operatorHeader.ModifiedOn = DateTime.Now;
        //        operatorHeader.ModifiedBy = 1;// get from session once these screens are integrated....
        //        operatorHeader.MachineMode = ModeType;// updating current mode
        ...
        {
            public int ToolActivityID { get; set; }
            public string Toolactivityno { get; set; } 
        }

        public class ToolactivitySubTask
        {
